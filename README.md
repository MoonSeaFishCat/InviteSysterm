...
- **🛡️ 星月御安全系统**：
  ...
  - **大厂级设备指纹**：采集 WebGL、Audio、Font、屏幕硬件等 30+ 维度熵源，结合 MurmurHash3 哈希，精准识别真实设备
  - 时间戳验证（10 分钟有效期）
  - Nonce 防重放攻击
- **📧 安全分发系统**：
  - **零存储机制**：系统数据库不存储任何邀请码，生成后即刻通过邮件发放，物理隔离倒卖风险
  - **数据脱敏**：角色化权限控制，非超级管理员无法查看用户真实邮箱
- **🤖 人机验证**：集成极验 4.0 Captcha
- **📧 邮箱验证**：SMTP 验证码 + 密码重置功能
- **🚫 风控系统**：
  - **三重联合校验**：邮箱 + 设备指纹 + 用户ID 三位一体拦截重复申请
  - IP/设备/邮箱黑名单
  - 邮箱白名单
  - 申请频率限制
  - 速率限制（60 req/min）
...
✅ **高级设备指纹**：工业级多维度特征采集（WebGL/Audio/Font）与 MurmurHash3 处理
✅ **邀请码零存储**：不留存任何邀请码数据，全程通过加密邮件瞬时分发
✅ **角色化脱敏**：敏感数据（如邮箱）仅限超级管理员可见
✅ **三重风控拦截**：邮箱、设备ID、用户ID 三位一体防刷机制
✅ **密钥管理**：自动生成 + 24 小时轮换
✅ **密码重置**：安全的邮箱验证流程
✅ **速率限制**：全局 + 业务级限制
✅ **黑名单系统**：IP/设备/邮箱多维度防护
...
- **项目主页**: [GitHub Repository](https://github.com/MoonSeaFishCat/InviteSysterm)
- **问题反馈**: [Issues](https://github.com/MoonSeaFishCat/InviteSysterm/issues)
